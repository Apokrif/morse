...
public class AdFetchTask extends AsyncTask<String, Void, AdLoadTask> {
    ...
    public AdFetchTask(TaskTracker taskTracker, Engine adViewController, String userAgent, int timeoutMilliseconds) {
    ...
    protected void onPostExecute(AdLoadTask adLoadTask) {
        ...
        if (mAdViewController == null || mAdViewController.isDestroyed()) {
        	Logger.d("cleanup() has already been called on the AdViewController, don't proceed.");
        	if (adLoadTask != null) {
                adLoadTask.cleanup();
            }
        ...
        if (adLoadTask == null) {
             ...
             *
             * Note: We place the exponential back off after AdViewController's adDidFail because we only
